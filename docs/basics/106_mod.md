...
Se invece di scrivere i test a mano vogliamo usare uno strumento semplice ed automatico, ecco il modulo `doctest`!!!! Questo *esegue* gli esempi che trova nella documentazione come se ci fosse un interprete a farli!!
Infatti, per forzare doctest a *generare* degli esempi da verificare, occorrono due condizioni:

1. i test devono essere... (finiscimi)
2. inventami
