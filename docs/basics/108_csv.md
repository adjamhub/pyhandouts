...
perché sono finiti i tipi di parentesi esistenti :smile: <br>
...
Chiedere all'utente un prodotto e restituire il numero di elementi dello stesso presenti nel magazzino. 
Ovviamente, se l'elemento non è presente si scriva ZERO (sugg: usate la funzione…).
Chiedere all'utente di selezionare un altro prodotto e stavolta, se il prodotto è presente (va verificato con…) togliere quel prodotto dal magazzino (usate la funzione…), 
visualizzando anche il numero di pezzi che vengono scaricati.
...
*Step 3*
:   Caricare i dati dal file CSV in una apposita struttura dati ed estrapolare i cinema inseriti dopo il 2015 per la regione "Lombardia". 
Con i suddetti dati creare un nuovo file CSV chiamato "LombardiaPost2015.csv" con i dati delle colonne "Nome", "Comune", "Provincia", "Longitudine", "Latitudine".
...
*Step 1*
:   Come l'esercizio 671, ma creando nella home dell'utente il file "MonumentiPuglia.txt" (se la regione selezionata è la Puglia) con i
monumenti elencati uno per riga (ripeto, nella stessa visualizzazione sopra).
